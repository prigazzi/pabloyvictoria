...
exports.config = {
  ...
  ###
  modules: ['lint', 'server', 'require', 'minify', 'live-reload', 'combine', 'server-template-compile']
  ...
  ###
  copy:
    extensions: ["php","js","css","png","jpg","jpeg","gif","html","eot","svg","ttf","woff","otf","yaml","kml","ico","htc","htm","json","txt","xml","xsd"]	
  ...
  server:                      # configuration for server when server option is enabled via CLI
    ...
    views:                     # configuration for the view layer of your application
      compileWith: 'hogan'      # Valid options: "jade", "hogan", "html", "ejs", "handlebars".
  ...
  serverTemplate:
    ...
    locals:                         # a list of properties to pass into all the templates when
      ...
      hide_reload: true                 # flag for whether or not to include live reload, which does
